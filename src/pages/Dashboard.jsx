...
import { Target, TrendingUp, DollarSign, Percent, BarChart2, AlertTriangle, CheckCircle, Zap, Award } from 'lucide-react'
...
export default function Dashboard() {
    ...
    if (isManager) {
      Promise.all([
        ...
        .then(([rows, anal]) => {
          ...
          const totalMoneyMade  = totalCommission + totalT2Amount
          setSummary({
            totalTarget,
            totalAchieved,
            totalCommission,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Tab switcher (agent only) ── */}
      {!isManager && (
        <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
          {['overview', 'incentives'].map(tab => (
            <button
              key={tab}
            ...
            >
              {tab === 'incentives' ? '💰 Incentives' : 'Overview'}
            </button>
          ))}
        </div>
      )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <MetricsCard
                  ...
                  icon={DollarSign}
                  color="purple"
                  sub={summary?.slabInfo?.eligible ? 'Slab eligible ✓' : summary?.slabInfo ? `₹${Math.ceil((summary.slabInfo.gapToSlab1 ?? 0) / 1000)}k to Slab 1` : undefined}
                />
                <MetricsCard
